...
#[no_mangle]
pub unsafe extern "C" fn stream_serde__init(ptr: *const c_char, len: size_t) {
    GLOBAL.json = Some(
        std::str::from_utf8_unchecked(slice::from_raw_parts(ptr as *const u8, len)).to_string(),
    );
}

#[no_mangle]
pub unsafe extern "C" fn stream_serde__prerun() {}

#[allow(static_mut_refs)]
#[no_mangle]
pub unsafe extern "C" fn stream_serde__run() {
    let path = GLOBAL.json.as_ref().unwrap();
    let file = File::open(path).unwrap();
    let reader = BufReader::new(file);
    GLOBAL.doc = Some(serde_json::from_reader(reader).unwrap());
}

#[no_mangle]
pub unsafe extern "C" fn stream_serde__postrun() {}

#[no_mangle]
pub unsafe extern "C" fn stream_serde__deinit() {
    GLOBAL.json = None;
    GLOBAL.doc = None;
}

#[no_mangle]
pub unsafe extern "C" fn stream_serde__memusage() -> size_t {
    0
}
